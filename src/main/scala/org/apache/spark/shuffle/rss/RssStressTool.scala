...
class RssStressTool extends Logging {
  ...
  def setup(): Unit = {
    ...
    (0 until numServers).foreach(i => {
      ...
      serverDetails.add(new ServerDetail(server.getServerId, server.getRunningVersion, s"localhost:${server.getShufflePort}"))
    })
  ...
  private def startNewServer(registryServer: String): StreamServer = {
    ...
    server.run()
    logInfo(s"Started server, port: ${server.getShufflePort}, rootDir: $serverDirFullPath, $serverConfig")
    server
  }
...
object RssStressTool extends Logging {
  ...
  private def runOnce(): Unit = {
    ...
    logInfo(s"Running test, numServers: ${tool.numServers}, " +
      s"numMaps: ${tool.numMaps}, numPartitions: ${tool.numPartitions}, numSplits: ${tool.numSplits}, " +
