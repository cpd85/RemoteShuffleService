...
import com.uber.rss.metadata.{ServiceRegistry, StandaloneServiceRegistryClient}
...
class RssShuffleManagerWithStandaloneRegistryServerTest {
  ...
  def runSparkApplication(): Unit = {
    ...
    conf.set("spark.shuffle.rss.serviceRegistry.server", s"localhost:${testStreamServer.getShufflePort}")
  ...
  def unreachableHost(): Unit = {
    ...
    val serviceRegistryClient = new StandaloneServiceRegistryClient("localhost", testStreamServer.getShufflePort, TestConstants.NETWORK_TIMEOUT, "user1");
    serviceRegistryClient.registerServer(ServiceRegistry.DEFAULT_DATA_CENTER, appId, "unreachable_server", "1", "unreachable_server_host_abc:9999")
